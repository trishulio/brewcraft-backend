...
CREATE TABLE MIXTURE_PORTION (
    ...
    MIXTURE_ID INTEGER REFERENCES MIXTURE(ID),
    QUANTITY_VALUE NUMERIC NOT NULL,
    QUANTITY_UNIT VARCHAR(4) REFERENCES QTY_UNIT(SYMBOL) NOT NULL,
    ADDED_AT TIMESTAMP,
    CREATED_AT TIMESTAMP DEFAULT CURRENT_TIMESTAMP NOT NULL,
    LAST_UPDATED TIMESTAMP DEFAULT CURRENT_TIMESTAMP NOT NULL,
    VERSION INTEGER
...
CREATE TABLE MATERIAL_PORTION (
    ...
    QUANTITY_VALUE_IN_SYS_UNIT NUMERIC NOT NULL,
    DISPLAY_QUANTITY_UNIT_SYMBOL VARCHAR(4) REFERENCES QTY_UNIT(SYMBOL),
    ADDED_AT TIMESTAMP,
    CREATED_AT TIMESTAMP DEFAULT CURRENT_TIMESTAMP NOT NULL,
    LAST_UPDATED TIMESTAMP DEFAULT CURRENT_TIMESTAMP NOT NULL,
    VERSION INTEGER
...
CREATE VIEW STOCK_LOT AS
    SELECT L.ID, L.LOT_NUMBER, L.DISPLAY_QTY_UNIT_SYMBOL, (L.QTY_VALUE_IN_SYS_UNIT - COALESCE(P.QUANTITY_USED_IN_SYS_UNIT, 0)) as QTY_VALUE_IN_SYS_UNIT, L.MATERIAL_ID, L.SHIPMENT_ID, L.INVOICE_ITEM_ID, L.STORAGE_ID, L.CREATED_AT, L.LAST_UPDATED, L.VERSION
    FROM PROCUREMENT_LOT AS L
        LEFT JOIN (SELECT MATERIAL_LOT_ID, SUM(QUANTITY_VALUE_IN_SYS_UNIT) AS QUANTITY_USED_IN_SYS_UNIT FROM MATERIAL_PORTION GROUP BY MATERIAL_LOT_ID) AS P
