...
CREATE TABLE INVOICE (
    ...
    TOTAL_AMOUNT_CURRENCY_CODE INTEGER REFERENCES CURRENCY(NUMERIC_CODE),
    TOTAL_AMOUNT NUMERIC,
    SUB_TOTAL_AMOUNT_CURRENCY_CODE INTEGER REFERENCES CURRENCY(NUMERIC_CODE),
    SUB_TOTAL_AMOUNT NUMERIC,
    PST_AMOUNT_CURRENCY_CODE INTEGER REFERENCES CURRENCY(NUMERIC_CODE),
    PST_AMOUNT NUMERIC,
    GST_AMOUNT_CURRENCY_CODE INTEGER REFERENCES CURRENCY(NUMERIC_CODE),
    GST_AMOUNT NUMERIC,
    HST_AMOUNT_CURRENCY_CODE INTEGER REFERENCES CURRENCY(NUMERIC_CODE),
    HST_AMOUNT NUMERIC,
    TOTAL_TAX_AMOUNT_CURRENCY_CODE INTEGER REFERENCES CURRENCY(NUMERIC_CODE),
...
(6, 'Whirlpool', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, 1),
(7, 'Barrel', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, 1),
(8, 'Brite Tank', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, 1),
(9, 'Mash Lauter Tun', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, 1),
(10, 'Mash', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, 1),
(11, 'Lauter Tun', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, 1),
(12, 'Conditioner', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, 1);
CREATE SEQUENCE EQUIPMENT_TYPE_SEQUENCE START 13;

CREATE TABLE EQUIPMENT (
    ID INTEGER PRIMARY KEY,
    FACILITY_ID INTEGER REFERENCES FACILITY(ID),
    NAME VARCHAR(255) NOT NULL,
    EQUIPMENT_TYPE_ID INTEGER REFERENCES EQUIPMENT_TYPE(ID),
    STATUS VARCHAR(255),
    MAX_CAPACITY_DISPLAY_UNIT_SYMBOL VARCHAR(4) REFERENCES QTY_UNIT(SYMBOL),
    MAX_CAPACITY_VALUE_IN_SYS_UNIT NUMERIC NOT NULL,
    CREATED_AT TIMESTAMP NOT NULL,
    LAST_UPDATED TIMESTAMP NOT NULL,
    VERSION INTEGER,
...
CREATE TABLE BREW (
    ...
    ENDED_AT TIMESTAMP,
    ASSIGNED_TO_USER_ID INTEGER REFERENCES _USER(ID),
    OWNED_BY_USER_ID INTEGER REFERENCES _USER(ID),
...
CREATE VIEW PROCUREMENT_LOT AS
    SELECT L.ID, L.LOT_NUMBER, M.UNIT_SYMBOL AS DISPLAY_QTY_UNIT_SYMBOL, COALESCE(L.QTY_VALUE_IN_SYS_UNIT, 0) AS QTY_VALUE_IN_SYS_UNIT, I.MATERIAL_ID, L.SHIPMENT_ID, L.INVOICE_ITEM_ID, L.STORAGE_ID, L.CREATED_AT, L.LAST_UPDATED, L.VERSION
...
CREATE VIEW FINISHED_GOOD_INVENTORY AS
    SELECT FGL.ID, FGL.SKU_ID, FGL.DISPLAY_QTY_UNIT_SYMBOL, (FGL.QTY_VALUE_IN_SYS_UNIT - COALESCE(FGLP.QTY_USED_IN_SYS_UNIT, 0)) as QTY_USED_IN_SYS_UNIT, FGL.PACKAGED_ON
