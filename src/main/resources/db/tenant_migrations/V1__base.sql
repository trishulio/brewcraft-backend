...
CREATE TABLE FINISHED_GOOD_LOT ( -- 1 fg lot has many fg_portions
    ID INTEGER PRIMARY KEY,  --1,,,2
    SKU_ID INTEGER NOT NULL REFERENCES SKU(ID),  --1(can),,,2(box)
    PACKAGED_ON TIMESTAMP NOT NULL,
    DISPLAY_QTY_UNIT_SYMBOL VARCHAR(4) REFERENCES QTY_UNIT(SYMBOL),
    QTY_VALUE_IN_SYS_UNIT NUMERIC NOT NULL, --100(can),,,,1(box)
    CREATED_AT TIMESTAMP DEFAULT CURRENT_TIMESTAMP NOT NULL,
    LAST_UPDATED TIMESTAMP DEFAULT CURRENT_TIMESTAMP NOT NULL,
    VERSION INTEGER NOT NULL
);
...
CREATE TABLE FINISHED_GOOD_LOT_PORTION (
    ...
    FINISHED_GOOD_LOT_ID INTEGER NOT NULL REFERENCES FINISHED_GOOD_LOT(ID), --1
    DISPLAY_QTY_UNIT_SYMBOL VARCHAR(4) REFERENCES QTY_UNIT(SYMBOL),
    QTY_VALUE_IN_SYS_UNIT NUMERIC NOT NULL, --10(cans)
    CREATED_AT TIMESTAMP DEFAULT CURRENT_TIMESTAMP NOT NULL,
    LAST_UPDATED TIMESTAMP DEFAULT CURRENT_TIMESTAMP NOT NULL,
    VERSION INTEGER
);
...
CREATE TABLE FINISHED_GOOD_LOT_FINISHED_GOOD_LOT_PORTION (
    FINISHED_GOOD_LOT_ID INTEGER REFERENCES FINISHED_GOOD_LOT(ID) ON DELETE CASCADE, -- 2(BOX)
    FINISHED_GOOD_LOT_PORTION_ID INTEGER REFERENCES FINISHED_GOOD_LOT_PORTION(ID), --1(CAN)
...
CREATE VIEW FINISHED_GOOD_INVENTORY AS
    SELECT FGL.ID, FGL.SKU_ID, (FGL.QTY_VALUE_IN_SYS_UNIT - COALESCE(FGLP.QTY_USED_IN_SYS_UNIT, 0)) as QUANTITY
